...
import { completeDriveOAuth, verifyDriveOAuthState } from '@/lib/connectors/drive';
...
export async function GET(req: Request): Promise<Response> {
  ...
  const stateOrg = verifyDriveOAuthState(url.searchParams.get('state'));
  ...
  const redirectUri = `${url.origin}/api/connectors/drive/oauth/callback`;
  ...
  redirect('/dashboard/connectors?connected=drive');
}
