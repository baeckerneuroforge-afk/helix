...
export interface FrameworkCriteriaThresholds {
  min_use_cases?: number;
  min_length?: number;
}

/** Build framework criteria with optional numeric threshold overrides. */
export function buildFrameworkCriteria(
  overrides: FrameworkCriteriaThresholds = {},
): AcceptanceCriteriaSet {
  const minUC = overrides.min_use_cases ?? MIN_USE_CASES;
  const minLen = overrides.min_length ?? MIN_LENGTH_CHARS;
        ...
        detail: {
          ...
          message:
            count >= minUC
              ...
              : `Found ${count} use case(s), expected at least ${minUC}`,
        },
      };
    },
  };
  ...
  const hasExecutiveSummary: AcceptanceCriterion = {
    ...
    check: (obs: Observation): CriterionResult => {
      ...
      const found = headings.some((h) => h.toLowerCase() === 'executive summary');
      return {
        key: 'has_executive_summary',
        passed: found,
        detail: {
  ...
  const hasAllSections: AcceptanceCriterion = {
    ...
    check: (obs: Observation): CriterionResult => {
      const headings = extractH2Headings(obs.content ?? '').map((h) => h.toLowerCase());
      const required = FRAMEWORK_SECTIONS.en.length;
      let matched = 0;
      const missing: string[] = [];

      for (const en of FRAMEWORK_SECTIONS.en) {
        const de = FRAMEWORK_SECTIONS.de[FRAMEWORK_SECTIONS.en.indexOf(en)] ?? en;
        if (headings.some((h) => h === en.toLowerCase() || h === de.toLowerCase())) {
          matched++;
        } else {
          missing.push(en);
        }
        ...
        detail: {
          ...
          message:
            length >= minLen
              ...
              : `Content has ${length} non-whitespace characters, expected at least ${minLen}`,
        },
      };
    },
  };
...
export const frameworkCriteria: AcceptanceCriteriaSet = buildFrameworkCriteria();
